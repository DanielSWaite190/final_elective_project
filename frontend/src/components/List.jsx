...
// const dimage = "../media/default_halo_image"
...
const ews = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmb3VuZFVzZXIiOnsidXNlcm5hbWUiOiJkYW5pZWwiLCJwYXNzd29yZCI6IiQyYiQxMCRkWjZzWFhUL0w4NFg1VmhYSXlTMEouVTR2REJpbkY4TGdidy9DTXdObW5xOE94UC9YVlJ2bSJ9LCJpYXQiOjE2ODY5NTExMzl9.y8ZP1ivWFNzMMF2tYI8toVtqzxEuRbdBDq9yG4QhRMU'
...
function List() {
    ...
    useEffect(() =>  {
        // fetch('http://localhost:5000/ring', {Authorization: `Bearer ${test}`})
        fetch('http://localhost:5000/ring', {Authorization: {ews}})
        .then((res) => res.json())
        .then((data) => {
            setRing(data);
        })
  ...
  return (
    <React.Fragment>
        <div id="container">
            <ul id="list">
                {ring?.map((halo) => (
                <Card key={halo.id} className="card" style={{ width: '18rem' }}>
